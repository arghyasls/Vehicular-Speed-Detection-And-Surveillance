# -*- coding: utf-8 -*-
"""
Created on Sat Apr  7 15:02:22 2018
...
cv2.moveWindow("Surveilance Camera", 10, 40)

# call the draw_rectangle routines when the mouse is used
cv2.setMouseCallback('Surveilance Camera',draw_rectangle)
...
while not setup_complete:
    cv2.imshow("Surveilance Camera",image)
   ...
   # cv2.imshow('frame',img)
    ...
    if motion_found:
        ...
        if state != WAITING:
            state = WAITING
            direction = UNKNOWN
            text_on_image = 'Car Detection Starting'
            print(text_on_image)
